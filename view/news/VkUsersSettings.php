...
<html>
    ...
    <section class=" news-section">
        <h1 class="h1">Поиск по записям пользователей ВК(<?php 
        ...
        <div id="DistrictSectionConfirm">
            <h2 class="srch-h2 pers-h2 h2">Добавить группу</h2><input id="NewGroup" name="Stop_word_inp" type="text" class="srch_panel pers-input" placeholder="Введите id группы или ее Screenname"><span id="AddNewVkGroupSettings" class="srch_ok ok" title="Подтвердить изменения">N</span>
        </div>
       
        <div id="search-panel" class="post">
                    <div  class="selectGr">
                    <h2 class="h2-distr">Группы</h2>
                    <ul id="GroupsOrder" class="chng_distr_div">
                        
                        <?php   
                        if(count($this->view->vk_users) == 0){
                            echo "Не найдено ни одного пользователя";
                        }//if
                        else{
                            foreach ($this->view->vk_users as $vk_user){
                                echo "<div><li data-stop-id=\"{$group->id}\" class=\"chng_distr_li\">{$vk_user->Id}<span class=\"chng_distr_correct correct\" title=\"Изменить\">M</span></li>";
                                echo "<div class=\"hg_null\"><input id=\"\" type=\"text\" class=\"chng_group_inp pers-input\" placeholder=\"Редактирование пользователя слова\">"
                    ...
                    <a href="vk_groups_queries.php"><input class="groups_button submit" value="Найти" type="button"></a>
